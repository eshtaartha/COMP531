...
export const goToProfile=function() 
{
    ...
    return (dispatch)=>
  {
    ...
    .catch(r => console.log(`"${r.message}" when fetching email`) )

    .then(r=>zipcode())
    .then(r=>{profileData.zipcode=r.zipcode})
    .catch(r => console.log(`"${r.message}" when fetching zipcode`) )

    .then(r=>dob())
    .then(r=>{profileData.dob=r.dob})
    .catch(r => console.log(`"${r.message}" when fetching date of birth`) ) 
