...
<div>
  ...
  <input type="file" className="profilePicUpdate"/>
  ...
  <label for="tel_input" className="profile_font_right">Phone number (Format: ########## or ###-###-####)</label>
  ...
  <input type="submit" className="updateButton" value="Update" id="update" onclick="updateProfile()"/>
...
{
  return bindActionCreators({goHome:goHome},dispatch)
}
...
function mapStateToProps(myState)
{
  return{
          ...
          email:myState.view.data.email,
          zipcode:myState.view.data.zipcode,
          dob:myState.view.data.dob
